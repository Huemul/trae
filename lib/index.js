...
class Trae {
  ...
  _fetch(url, originalConfig) {
    ...
    return this._middleware
      ...
      .then(
        ...
        (err) => Promise.resolve(onceDone(config, url)).then(() => Promise.reject(err)),
      )
  }
  ...
  _setupMethodsWithBody() {
    ...
    this.post = (path, body = {}, config = {}) => fetch(path, body, config, 'post')
    this.put = (path, body = {}, config = {}) => fetch(path, body, config, 'put')
    this.patch = (path, body = {}, config = {}) => fetch(path, body, config, 'patch')
  }
